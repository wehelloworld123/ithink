...
import com.fc.ithink.common.log.Log;
import com.fc.ithink.model.auto.SysDepartment;
import com.fc.ithink.model.auto.SysDepartmentExample;
import com.fc.ithink.model.auto.SysPosition;
import com.fc.ithink.model.auto.SysPositionExample;
...
import com.fc.ithink.service.SysDepartmentService;
import com.fc.ithink.service.SysPositionService;
...
public class UserController extends BaseController{
	 ...
	 * @return
	 */
	//@Log(title = "禁言用户", action = "1")
	@ApiOperation(value = "禁言", notes = "禁言")
	@PostMapping("/forbid")
	@RequiresPermissions("system:user:forbid")
	@ResponseBody
	public AjaxResult forbid(String ids,String fobid){
		TsysUser data = new TsysUser();
		data.setId(ids);;
		int b =0;
		if(StringUtils.isNotEmpty(fobid)&&"1".equals(fobid)) {
			data.setWriteAuth("0");
			b=sysUserService.updateUserWriteAuth(data);
		}else{
			data.setWriteAuth("1");
			b=sysUserService.updateUserWriteAuth(data);
		}
	...
	@ResponseBody
	public AjaxResult remove(String ids,String del){
		int b =0;
		if(StringUtils.isNotEmpty(del)&&"1".equals(del)) {
			b = sysUserService.deleteUserByPrimaryKey(ids);
		}else{
			b = sysUserService.updateBatchByPrimaryKey(ids);
		}
		if(b>0){
			return success();
		}else{
