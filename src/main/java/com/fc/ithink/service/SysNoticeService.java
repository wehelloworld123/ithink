...
import com.fc.ithink.mapper.auto.SysNoticeMapper;
import com.fc.ithink.mapper.auto.SysNoticeUserMapper;
import com.fc.ithink.model.auto.*;
import com.fc.ithink.model.auto.SysNoticeUserExample.Criteria;
...
public class SysNoticeService implements BaseService<SysNotice, SysNoticeExample>{
	 ...
	 * @return
	 */
	public PageInfo<SysNotice> listByUserId(Tablepar tablepar,String title,String userId){
		...
		if(userId!=null&&!"".equals(userId)){
			...
			map.put("title",title);
			list= sysNoticeMapper.selectUserNoticeByUserId(map);
		}else{
			list = new ArrayList<SysNotice>();
		}
	...
	public int insertSelective(SysNotice record) {
		...
		//添加创建人
		record.setCreateUsername(ShiroUtils.getUser().getNickname());
		//添加创建时间
	...
	public void editUserState(String noticeid) {
		...
		SysNoticeUserExample sysNoticeUserExample=new SysNoticeUserExample();
		sysNoticeUserExample.createCriteria().andNoticeIdEqualTo(noticeid).andUserIdEqualTo(userId).andStateEqualTo(1);
		List<SysNoticeUser> noticeUsers= sysNoticeUserMapper.selectByExample(sysNoticeUserExample);
		if(noticeUsers==null||noticeUsers.size()==0){
			SysNoticeUser data = new SysNoticeUser(SnowflakeIdWorker.getUUID(),noticeid,userId,1);
			sysNoticeUserMapper.insert(data);
		}
