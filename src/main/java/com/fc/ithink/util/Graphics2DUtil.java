...
//        num = 90;
        ...
        //绘制中间的五角星
        g.setFont(new Font("SimSun", Font.BOLD, num));
        g.drawString("★", CENTERX-(num/2), CENTERY+(num/3));

        //添加姓名
        if(!StringUtils.isEmpty(centerName)) {
            g.setFont(new Font("SimSun", Font.LAYOUT_LEFT_TO_RIGHT, 30));// 写入签名
            g.drawString(centerName, CENTERX - (num1), CENTERY + (30 + 50));
        }
        //添加年份
        if(!StringUtils.isEmpty(year)) {
            g.setFont(new Font("SimSun", Font.LAYOUT_LEFT_TO_RIGHT, 20));// 写入签名
